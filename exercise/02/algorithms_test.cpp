...
public:
    ...
    v.assign(std::istream_iterator<int, wchar_t>(ss), std::istream_iterator<int, wchar_t>());
    ...
    std::generate(v.begin(), v.end(), IncrementingBy2);
    ...
    auto res = std::accumulate(v.begin(), v.end(), static_cast<std::wstring>(L"GO "));
    ...
    auto total_age = std::accumulate(v.begin(), v.end(), 0, [](int a, person& P) {return a + P.age; });
    ...
    auto number_of_negative = std::count_if(v.begin(), v.end(), [](int n) {return n < 0 ? true : false; });
    ...
    auto first_prime = *std::find_if(v.begin(), v.end(), [](int num)
    {bool IsPrime = false; for (int i = 2; i < num - 1; i++) { if (num%i == 0) return false; } return true; } );
    ...
    std::replace_if(v.begin(), v.end(), [](double num) {return num == 1e10 ? true : false; }, -1.0);
    ...
    std::replace_if(s.begin(), s.end(), [](wchar_t& letter)
    { return ((letter == L'a') || (letter == L'e') || (letter == L'i') || (letter == L'o') || (letter == L'u')); }, L'x');
    ...
    v.erase(std::remove_if(v.begin(), v.end(), [](double num) {return num == 1e10 ? true : false; }), v.end());
    ...
    s.erase(std::remove_if(s.begin(), s.end(), [](wchar_t& letter)
    { return ((letter == L'a') || (letter == L'e') || (letter == L'i') || (letter == L'o') || (letter == 'u')); }), s.end());
    ...
    std::sort(v.begin(), v.end(), [](const exam& ex1, const exam& ex2)
    { return ex1.grade == ex2.grade ? ex1.points > ex2.points : ex1.grade > ex2.grade; });
    ...
    std::nth_element(v.begin(), v.begin() + v.size() / 2, v.end(), 
      [](const employee& e1, const employee& e2) { return e1.salary < e2.salary; });
    ...
    std::adjacent_difference(atp_points.rbegin(), atp_points.rend(), atp_points.rbegin());
