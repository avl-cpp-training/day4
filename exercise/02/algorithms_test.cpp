...
public:
    ...
    std::generate(v.begin(), v.end(), []() { static int start = -1; return start += 2; });
		Assert::IsTrue(std::is_sorted(v.cbegin(), v.cend()));
    ...
    std::transform(v.begin(), v.end(), v.begin(), [](auto val) { return std::pow(val, 3); });
		Assert::AreEqual(3u, v.size());
		Assert::AreEqual(1, v[0]);
    ...
    std::transform(std::cbegin(x), std::cend(x), std::cbegin(y), std::back_inserter(d), [](auto x, auto y) { return std::sqrt(x * x + y * y); });
    ...
    auto total_age = std::accumulate(std::cbegin(v), std::cend(v), 0, [](int sum, auto&& x) { return sum + x.age; });
    ...
    auto number_of_negative = std::count_if(std::cbegin(v), std::cend(v), [](auto x) { return x < 0; });
    ...
    auto number_of_invalid = std::count(std::cbegin(v), std::cend(v), 1e10);
    ...
    auto number_in_first_quadrant = std::count_if(std::cbegin(v), std::cend(v), [](auto&& point) { return point.x >= 0 && point.y >= 0; });
    ...
    auto first_prime = *std::find_if(v.cbegin(), v.cend(), [](auto val)
    {
      bool is_prime = true;
      for (int i = 2; i <= static_cast<int>(std::sqrt(val)); ++i)
      {
        if (0 == val % i)
        {
          is_prime = false;
  ...
  TEST_METHOD(test_07b)
  {
    ...
    std::replace_if(s.begin(), s.end(), [](auto c) { return c == L'a' || c == L'e' || c == L'i' || c == L'o' || c == L'u'; }, L'x');
    ...
    s.erase(std::remove_if(s.begin(), s.end(), [](auto c) { return c == L'a' || c == L'e' || c == L'i' || c == L'o' || c == L'u'; }), s.end());
    ...
    std::vector<int> tmp;
    tmp.reserve(atp_points.size());
    std::adjacent_difference(atp_points.crbegin(), atp_points.crend(), std::back_inserter(tmp));
    auto smallest_difference = *std::min_element(tmp.cbegin()+1, tmp.cend());
