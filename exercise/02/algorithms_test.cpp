...
#include <numeric>
#include <chrono>
auto is_vowel = [](auto const& chr)
{
  static const std::wstring set{L"AEIOUaeiou"};
...
public:
  TEST_METHOD(test_01)
  {
    ...
    std::vector<int> v{}; // TODO: read values from input stream into vector
  ...
  {
    std::vector<int> v(10); // TODO: fill vector with incremental values
  ...
  {
    // generate
    std::vector<int> v(10); // TODO: fill vector with incremental values (by 2)
  ...
  {
    std::vector<int> v = {1, 5, 10}; // TODO: change all values in a vector
  ...
  TEST_METHOD(test_03b)
  {
    ...
    std::vector<double> d; // TODO: calculate distances from origin (from x and y collections) to new vector
  ...
  {
    std::wstringstream ss(L"1.5 2.5 3.5"); // TODO: sum of all values in input stream
  ...
  {
    std::vector<std::wstring> v{L"A", L"V", L"L", L"!"}; // TODO: concatenated string with additional prefix 
  ...
  TEST_METHOD(test_04c)
  {
    ...
    std::vector<person> v{{L"Pero", 33}, {L"Iva", 25}}; // TODO: sum of all ages
  ...
  {
    std::vector<double> v{1.5, 8, -11.23, 0, 1e10, 1e10, 1e10, 0, 99}; //condition not defined
    auto number_of_invalid = std::count(v.cbegin(), v.cend(), 1e10);
  ...
  {
    std::vector<double> v{1e10, 8, -11.23, 0, 1e10, 1e10, 1e10, 0, 99}; // TODO: change every invalid value (1e10) with -1 
  ...
  {
    std::wstring s(L"ponedjeljak"); // TODO: change every vowel with x 
    std::replace_if(s.begin(), s.end(), is_vowel, L'x');
    Assert::AreEqual(L"pxnxdjxljxk", s.c_str());
  }

  TEST_METHOD(test_08a)
  {
    std::vector<double> v{1e10, 8, -11.23, 0, 1e10, 1e10, 1e10, 0, 99}; // TODO: delete all invalid values (1e10)
  ...
  {
    std::wstring s(L"ponedjeljak"); // TODO: delete all vowels 
  ...
  TEST_METHOD(test_09)
  {
    ...
    std::vector<exam> v{{L"Pero", 55, 2}, {L"Iva", 93, 5}, {L"Marko", 89, 5}}; // TODO: sort vector by grade, then by points
    std::sort(v.begin(), v.end(), [](auto const& lhs, auto const& rhs) { return lhs.points > rhs.points; });
    std::stable_sort(v.begin(), v.end(), [](auto const& lhs, auto const& rhs) { return lhs.grade > rhs.grade; });
  ...
  {
    // nth_element
    std::vector<double> v(2e7); // half of the values less than 1000
    std::generate(v.begin(), v.begin() + v.size() / 2, []() { return rand() % 1000; }); // other half of the values greater than 1000
    std::generate(v.begin() + v.size() / 2, v.end(), []() { return 1001 + rand() % 1000; });
    v.push_back(1000); // to be median
    std::random_shuffle(v.begin(), v.end());
    using namespace std::chrono;
    auto t1 = steady_clock::now(); // TODO: put median value in the middle of vector. fast.
  ...
  TEST_METHOD(test_10b)
  {
    ...
    std::vector<employee> v{{L"Iva", 2000}, {L"Pero", 1000}, {L"Marko", 10000}}; // TODO: put employee with median salary in the middle of vector
  ...
  TEST_METHOD(test_12)
  {
    ...
    auto min_max = std::minmax_element(v.cbegin(), v.cend());
    auto smallest_value = *min_max.first;
    Assert::AreEqual(-97.23, smallest_value);
    auto largest_value = *min_max.second;
    Assert::AreEqual(48.78, largest_value);
  }

  TEST_METHOD(test_13)
  {
    std::vector<int> atp_points{8445, 7480, 6220, 5300, 5285}; // the most interesting match is the one with the smallest difference
