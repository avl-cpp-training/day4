...
public:
	...
	TEST_METHOD(test_04a)
	{
		...
		auto res = // sum of all values in input stream
		Assert::AreEqual(7.5, res);
	}
	TEST_METHOD(test_04b)
	{
		std::vector<std::wstring> v { L"A", L"V", L"L", L"!" };
		auto res = // concatenated string with additional prefix 
	...
	TEST_METHOD(test_04c)
	{
		...
		auto total_age = // sum of all ages
	...
	TEST_METHOD(test_07a)
	{
		...
		// change every invalid value with -1 
		Assert::AreEqual(-1., v[0]);
